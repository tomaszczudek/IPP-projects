...
# ------------------------------------------------------------
# Parser pro jazyk SOL25
# Autor: Tomasz Czudek - xczudet00
# Popis: Tento skript slouží k analýze a zpracování
# SOL25 programu a generování odpovídající XML reprezentace.
# ------------------------------------------------------------
...
# Vytvoření argument parseru pro zpracování vstupních argumentů
parser_arg = argparse.ArgumentParser(
    ...
    add_help=False  # Zakáže výchozí nápovědu
)
...
try:
    ...
    # Pokud dojde k chybě při parsování argumentů, vypíšeme chybu a ukončíme program
    print("Error: Invalid arguments", file=sys.stderr)
    sys.exit(10)

# Pokud uživatel zadá argument -h nebo --help
if args.help:
    if len(sys.argv) > 2:
        # Pokud jsou kromě -h ještě jiné argumenty, považujeme to za chybu
        print("Error: Invalid arguments", file=sys.stderr)
        sys.exit(10)
    
    # Vypíše nápovědu a ukončíme program
    parser_arg.print_help()
    sys.exit(0)

# Definice gramatiky pro jazyk SOL25
grammar = r'''
    program: classdec* | 
...
# Načtení vstupního kódu ze standardního vstupu
try:
...
# Extrahování komentářů (řetězců v uvozovkách) a odstranění z kódu
comments = re.findall(r'"(.*?)"', input_code, re.DOTALL)
input_code = re.sub(r'"(.*?)"', ' ', input_code, flags=re.DOTALL)

# Pokus o parsování vstupního kódu podle gramatiky
try:
...
# Hlavní funkce pro zpracování AST a jeho převod do XML.  
# Rekurzivně volá pomocné funkce pro jednotlivé uzly.  
def parse_program(program_node):
    ...
    # Procházení všech uzlů programu a parsování deklarací tříd
...
# Převádí deklaraci třídy na XML <class>
...
def parse_block_stat(block_stat_node, order, local_vars, formal_vars):
    ...
    # Uložení lokální proměnné
    local_vars.append(var_name)

    return xml_elem

# Převádí výraz na XML <expr>
def parse_expr(expr_node, local_v):
    ...
    if len(expr_node.children) == 1:
        ...
        if isinstance(child, lark.tree.Tree) and hasattr(child, 'children') and len(child.children) > 0:
            if isinstance(child.children[0], lark.lexer.Token):
                ...
                # Pokud je uzel blokem, zpracujeme ho rekurzivně
                ...
                # Pokud je uzel výraz v závorkách
    ...
    # Zpracování metody
        ...
        # Kontrola, zda jde o unární zprávu
        if len(expr_node.children) == 2 and isinstance(expr_node.children[1], lark.lexer.Token) and expr_node.children[1].type == 'ID':
            ...
            if isinstance(expr_node.children[0], lark.lexer.Token):
                ...
                # Rekurzivní zpracování vnořeného výrazu
            ...
            for idx, arg in enumerate(args):
                ...
                arg_elem.append(parse_expr(arg, local_v))
...
# Provádí statickou sémantickou kontrolu kódu v SOL25
def check_semantics():
    # Kontrola, zda existuje třída 'Main'
    if not any(class_name == "Main" for class_name, _ in class_info):
        print("Error: Missing class 'Main'", file=sys.stderr)
        sys.exit(31)

    # Kontrola, zda třída 'Main' obsahuje metodu 'run' s aritou 0
    if methods.get("Main", {}).get("run") != 0:
        print("Error: Class 'Main' must have method 'run' with arity 0", file=sys.stderr)
        sys.exit(31)
    
    # Kontrola, zda ve formálních a lokálních proměnných nedochází ke kolizím názvů
    for method, (formal_vars, local_vars) in colision_vars.items():
        collision = set(formal_vars) & set(local_vars)
        if collision:
            print(f"Error: Variable name collision '{collision}' in method '{method}'.", file=sys.stderr)
            sys.exit(34)

    class_dict = {class_name: parent for class_name, parent in class_info}
    # Kontrola, zda není v dědičnosti cyklus
    ...
    # Kontrola, zda všechny třídy mají platného rodiče
    ...
    # Kontrola, zda jsou ve výrazech použity pouze definované třídy
    ...
    # Kontrola, zda jsou použity pouze definované metody
...
# Klíčová slova v jazyce SOL25
key_words = {"class", "self", "super", "nil", "true", "false"}

# Vestavěné třídy a jejich metody
built_in = {
    # {Třída: [instanční metody]}
...
# Každá vestavěná třída (kromě Object) dědí metody z Object
for class_methods in built_in:
    if class_methods != "Object":
        built_in[class_methods] = built_in[class_methods] + built_in["Object"]

class_name = None  # Proměnná pro aktuálně zpracovávanou třídu

# Seznam pseudo-proměnných
pseaudo_var_sel = ["self", "super", "nil", "true", "false"]

# Struktury pro ukládání informací o třídách a metodách
class_info = []  # Seznam tříd a jejich rodičovských tříd [[class_name, parent_class_name]]
methods = {}  # Slovník metod ve třídách {class_name: {selector_name: arity}}
expr_selectors = []  # Seznam selektorů použitých ve výrazech [class_name, selector_name]
expr_class = []  # Seznam tříd použitých ve výrazech
colision_vars = {}  # Sledování proměnných pro detekci kolizí {method_name: [[formal_var], [local_var]]}

# Zpracování AST stromu
xml_document = parse_program(ast_tree)
# Kontrola sémantiky kódu
check_semantics()
# Vytvoření XML reprezentace AST stromu
xml_document = ET.tostring(xml_document, encoding="utf-8").decode("utf-8")

# Úprava XML
xml_document = xml.dom.minidom.parseString(xml_document).toprettyxml(indent="  ")
xml_document = re.sub(r'(</method>)(?=.*</method>)', r'\1\n', xml_document, flags=re.DOTALL)
xml_document = xml_document.replace("/>", " />").replace("\'", "&apos;").replace("&amp;quot;", "&quot;").replace("&amp;#10;", "&#10;")
xml_document = xml_document.replace("<?xml version=\"1.0\" ?>", "<?xml version=\"1.0\" encoding=\"UTF-8\"?>")

# Výstup XML na standardní výstup
try:
    print(xml_document, flush=True)
except (OSError, IOError):
